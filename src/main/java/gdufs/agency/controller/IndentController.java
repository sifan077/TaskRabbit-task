...
//					out.print(1);
                ...
                RequestDispatcher dispatcher = request.getRequestDispatcher("/user/updateBalance?openId=" + publishId + "&balance=-" + price);
    ...
    @RequestMapping(value = "/getIndents", produces = "text/json;charset=UTF-8")
...
//			System.out.println("before"+request.getCharacterEncoding());
        ...
        try {
            String publishId = request.getParameter("openId");
            // 找出不是自己的订单
            ...
            PrintWriter out = response.getWriter();
            Gson gson = new Gson();
            String resultString = gson.toJson(null);
            out.print(resultString);
        }
    }
...
//			System.out.println("before"+request.getCharacterEncoding());
        ...
        try {
            ...
            List<Indent> indent = indentService.getIndentsByType(openId, type);
    ...
    public void cancelIndentByPublishNotAccepted(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            if (result) {
//					out.print(1);
                RequestDispatcher dispatcher = request.getRequestDispatcher("/user/updateBalance?openId=" + openId + "&balance=" + price);
                dispatcher.forward(request, response);
            } else
                out.print(0);
        } catch (Exception e) {
